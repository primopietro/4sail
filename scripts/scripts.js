		...
		function() {
			...
			$('#stars li').on(
					...
					function() {
						...
						var ratingValue = parseInt($('#stars li.selected')
								.last().data('value'), 10);
						var msg = "Thanks! You rated this " + ratingValue
								+ " stars.";

						var id_rated = $("#theUser");

						var dataToSend = "id_rated=" + id_rated.attr("user_id")
								+ "&rating=" + ratingValue;
			...
			$('#watch_listUl li').on('click', function() {
				...
				$.ajax({
					...
					success : function(data) {
						if (data == "exist") {
							// alert("The item has been removed from your 'Watch
							// list'");
						} else {
							// alert(msg);
						}
...
$(document).on(
		...
		function() {
			...
			var messaged = "Did you receive my Paypal payment for " + itemTitle
					+ " ? If so, please mark item as sold.";
			...
			dataToSend += "object=" + object + "&messaged=" + messaged
					+ "&fk_user_to=" + fk_user_to + "&item_id=" + fk_item_id;
			// alert(dataToSend);
		...
		.on(
				...
				function() {
					...
					fill += "<h4 class='h4Margin'>Send message</h4>"
							+ "<label id='user_id' class='none'>"
							+ id_user
							+ "</label>"
							+ "<label id='item_id' class='none'>"
							+ id_item
							+ "</label>"
							+ "<form>"
...
$(document).on(
		...
		function() {
			...
			if (email.val() != '' && password.val() != '') {
				var dataToSend = 'email=' + email.val() + '&password='
						+ password.val();
				$.ajax({
...
$(document)
		.on(
				...
				function() {
					...
					var spanPassword = $(this).closest("form").find(
							"#errorPassword");

					var first_name = $(this).closest("form")
							.find("#first_name");
					var last_name = $(this).closest("form").find("#last_name");

					var email = $(this).closest("form").find("#email");
					var email_confirm = $(this).closest("form").find(
							"#email_confirm");

					var password = $(this).closest("form").find("#password");
					var password_confirm = $(this).closest("form").find(
							"#password_confirm");
					...
					if (first_name.val() != '' && last_name.val() != ''
						...
						if (email.val() == email_confirm.val()) {
							...
							spanEmail.html('');
							spanEmail
									.append('Email confirmation does not correspond');
						}
						...
						if (password.val() == password_confirm.val()) {
							...
							spanPassword.html('');
							spanPassword
									.append('Password confirmation does not correspond');
						}

						if (boolPassword == 1 && boolEmail == 1) {
							var dataToSend = 'first_name=' + first_name.val()
									+ '&last_name=' + last_name.val()
									+ '&email=' + email.val() + '&password='
									+ password.val() + '&phone=' + phone.val()
									+ '&address=' + address.val();
							$
									.ajax({
										url : ajaxPath
												+ 'actions/addAccount.php',
										type : 'POST',
										data : dataToSend,
										success : function(data) {
											if (data == "success") {
												...
												alert("Successful account creation. You can now login with: "
														+ email.val());
											}
										}
									});
...
$(document).on(
		...
		function() {
			...
			if (confirm("'Are you sure you want to mark this item as SOLD ?")) {
				...
				var dataToSend = "idItem=" + idItem + "&idRef=" + idRef
						+ "&idMsg=" + idMsg + "";
